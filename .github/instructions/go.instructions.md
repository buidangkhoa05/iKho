...
- Organize code by **feature (vertical slice)** rather than by technical layer
- Each feature slice lives in its own package under `internal/features/` (e.g., `internal/features/orders/`, `internal/features/products/`)
- A slice owns everything it needs: handler, service/logic, repository, DTOs, and validation — all co-located in one package
- Keep cross-cutting concerns (middleware, logging, auth) in shared packages like `internal/shared/` or `internal/middleware/`
- Define narrow interfaces at the slice boundary; accept interfaces, return concrete types
- Slices should not import from other slices directly — communicate through shared contracts or events when needed
- Each slice should be independently testable with its own `_test.go` files
- Use the `main` package in `cmd/` only for wiring (dependency injection, router setup, configuration)
- Typical slice structure:
  ```
  internal/features/<feature>/
    handler.go      # HTTP handler / transport layer
    service.go      # Business logic
    repository.go   # Data access
    models.go       # Request/response DTOs and domain types
    validation.go   # Input validation
    <feature>_test.go
  ```
- Prefer small, focused slices over large packages — split when a feature grows complex
- Shared domain types or value objects go in `internal/domain/`
